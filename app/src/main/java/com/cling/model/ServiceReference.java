...
/**
 * Combines a {@link com.cling.model.types.UDN} and a {@link com.cling.model.types.ServiceId}.
 * <p>
 * A service reference is useful to remember a service. For example, if a control point has accessed
 * a service one, it can remember the service with {@link com.cling.model.meta.Service#getReference()}.
 * Before every action invocation, it can now resolve the reference to an actually registered service with
 * {@link com.cling.registry.Registry#getService(ServiceReference)}. If the registry doesn't return
